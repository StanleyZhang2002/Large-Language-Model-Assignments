...
class TraceTransformer(HookedTransformer):
    ...
    def find_sequence_span(self, prompt: str, seq: str) -> Tensor:
        ...
        sequence_tokens = self.to_tokens(seq)
        
        offset = 1 if self.tokenizer.decode(sequence_tokens[0][0]) == '<|endoftext|>' else 0
        sequence_length = len(sequence_tokens[0]) - offset

        for index in range(offset, len(prompt_tokens[0]) - sequence_length + 1):
            found = True
            for i in range(sequence_length):
                if prompt_tokens[0][index + i] != sequence_tokens[0][offset + i]:
                    found = False
                    break
            if found:
                return torch.tensor(range(index, index + sequence_length))
        
        raise ValueError(f"The given sequence '{seq}' is not found in the given prompt.")
    ...
    def get_patch_emb_fn(self, corrupt_span: Tensor, noise: float = 1.) -> Callable:
        ...
        """
        ### YOUR CODE STARTS HERE
        def patch_embed_fn(embeddings, **kwargs):
            embeddings[:, corrupt_span, :] += noise * torch.randn_like(embeddings[:, corrupt_span, :])
            return embeddings
    ...
    def get_restore_fn(self,
        ...
        """
        ### YOUR CODE STARTS HERE
        def restore_fn(activations, hook):
    ...
    def get_forward_hooks(self, layer: int,
        ...
        """
        ### YOUR CODE STARTS HERE
        # embed once and restore multiple
        hooks = []
        ...
        for index in layers:
            hooks.append((utils.get_act_name(patch_name, layer=index), restore_fn))
    ...
    def causal_trace_analysis(self,
        ...
        P_star = corrupted_probs[target_id].item()
        ...
        for token_idx in range(1, seq_len):
            for layer in range(n_layers):
                restore_fn = self.get_restore_fn(activation_record, token_idx)
                hooks = self.get_forward_hooks(layer, patch_embed_fn, patch_name, restore_fn, window=window)
                logits = self.run_with_hooks(prompt_tokens, fwd_hooks=hooks)
                probs = torch.softmax(logits[0, -1, :], dim=-1)
                P_star_clean_li = probs[target_id].item()
                result[token_idx - 1, layer] = P_star_clean_li - P_star
        return result
