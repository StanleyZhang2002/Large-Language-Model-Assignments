    ...
    for batch_n in trange(0, len(corpus), bs, desc='gathering',
        ...
        for sentence in batch:
            ...
            for word_token in sentence:
                sentence_tokens.append(word_token.wordform)
            sentences.append(sentence_tokens)
        outputs, offsets = run_bert(sentences)
        ...
        for index in range(len(batch)):
            ...
            for i in range(len(offset)):
                ...
                if first == 0 and second == 0:
                    ...
                    aligned_offset.append([i, i + 1])
                else:
                    aligned_offset[len(aligned_offset) - 1][1] = i + 1
            for j in range(len(batch[index])):
                if batch[index][j].synsets:
                    vector = output[aligned_offset[j][0]:aligned_offset[j][1]].mean(dim=0)
                    for synset in batch[index][j].synsets:
                        sense_vectors[synset].append(vector)
    mean_dictionary = {}
    for synset, vectors in sense_vectors.items():
        mean_dictionary[synset] = torch.mean(torch.stack(vectors), dim=0)
    ...
    **IMPORTANT**: When computing the cosine similarities and finding the sense
    ...
    for index in range(len(batch)):
        ...
        for i in indices[index]:
            token = batch[index][i]
            if not wn.synsets(token.lemma):
                prediction.append(mfs(batch[index], i))
                continue
            valid_synsets = []
            for synset in wn.synsets(token.lemma):
                if synset.name() in sense_vectors:
                    valid_synsets.append(synset.name())
            if not valid_synsets:
                prediction.append(mfs(batch[index], i))
                continue
            correspond_sense_vectors = []
            for name in valid_synsets:
                correspond_sense_vectors.append(sense_vectors[name])
            sense_matrix = torch.stack(correspond_sense_vectors)
            token_output = output[aligned_offset[i][0]:aligned_offset[i][1]].mean(dim=0).unsqueeze(0)
            best = (torch.matmul(token_output, sense_matrix.T) / (token_output.norm(dim=1, keepdim=True) * sense_matrix.norm(dim=1, keepdim=True).T)).argmax()
            best_synset = valid_synsets[best]
            prediction.append(wn.synset(best_synset))
