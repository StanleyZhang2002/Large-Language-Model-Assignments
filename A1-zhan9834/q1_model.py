...
class ParserModel(nn.Module):
    ...
    def create_net_layers(self) -> None:
        ...
        """
        # *** ENTER YOUR CODE BELOW *** #
        self.hidden_layer = nn.Linear((self.config.n_word_features + self.config.n_tag_features + self.config.n_deprel_features) * self.config.embed_size, self.config.hidden_size)
    ...
    def reshape_embedded(self, input_batch: torch.Tensor) -> torch.Tensor:
        ...
        """
        # *** ENTER YOUR CODE BELOW *** #
        reshaped_batch = input_batch.view(input_batch.size(0), -1)
    ...
    def concat_embeddings(self, word_id_batch: torch.Tensor,
        ...
        """
        # *** ENTER YOUR CODE BELOW *** #
        word_embed_reshaped = self.reshape_embedded(self.word_embed(word_id_batch))
        tag_embed_reshaped = self.reshape_embedded(self.tag_embed(tag_id_batch))
        deprel_embed_reshaped = self.reshape_embedded(self.deprel_embed(deprel_id_batch))
        reshaped = torch.cat((word_embed_reshaped, tag_embed_reshaped, deprel_embed_reshaped), dim=1)
        return reshaped
    ...
    def forward(self,
        ...
        relu_h = torch.relu(h)
        h_drop = torch.dropout(relu_h, p=self.config.dropout, train=self.training)
