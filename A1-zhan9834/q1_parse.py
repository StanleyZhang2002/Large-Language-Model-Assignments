    ...
    def get_oracle(self, graph: DependencyGraph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        if(stack_size == 1):
            ...
            if self.next == len(self.sentence):
                transition, dep_rel_label = self.right_arc_id, get_dep_rel(self.stack[1], graph)
            else:
                transition, dep_rel_label = self.shift_id, None
        elif(stack_size >= 3):
            word1 = self.stack[-1]
            word2 = self.stack[-2]
            if get_head(word2, graph) == word1:
                transition, dep_rel_label = self.left_arc_id, get_dep_rel(word2, graph)
            elif get_head(word1, graph) == word2:
                transition, dep_rel_label = self.right_arc_id, get_dep_rel(word1, graph)
                for dependency in get_deps(word1, graph):
                    if dependency >= self.next or dependency in self.stack:
                        transition, dep_rel_label = self.shift_id, None
            else:
                transition, dep_rel_label = self.shift_id, None
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        for index in range(len(minibatch)):
            partialParse = minibatch[index]
            try:
                partialParse.parse_step(predictions[index][0], predictions[index][1])
                if(partialParse.complete):
                    parseToDelete.append(index)
            except ValueError:
                parseToDelete.append(index)
        for index in reversed(parseToDelete):
