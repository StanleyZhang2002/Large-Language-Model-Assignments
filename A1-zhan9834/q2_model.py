...
from q2_algorithm import is_single_root, mst_single_root
...
class GraphDepModel(nn.Module):
    ...
    def create_arc_layers(self) -> None:
        ...
        * Initialize self.arc_W and self.arc_B according to a uniform
        ...
        self.arc_h_mlp = nn.Sequential(
            nn.Linear(self.pt_width, self.cfg.n_arcs),
            nn.ReLU(),
            nn.Dropout(self.cfg.dropout),
            nn.Linear(self.cfg.n_arcs, self.cfg.n_arcs),
            nn.ReLU(),
            nn.Dropout(self.cfg.dropout)
        )
        ...
        self.arc_W = nn.Parameter(torch.empty(self.cfg.n_arcs, self.cfg.n_arcs))
        nn.init.uniform_(self.arc_W, -sqrt(3.0 / self.cfg.n_arcs), sqrt(3.0 / self.cfg.n_arcs))
        self.arc_B = nn.Parameter(torch.empty(self.cfg.n_arcs))
        nn.init.uniform_(self.arc_B, -sqrt(3.0 / self.cfg.n_arcs), sqrt(3.0 / self.cfg.n_arcs))
    ...
    def score_arcs(self, arc_head: Tensor, arc_dep: Tensor) -> Tensor:
        ...
        receive in this method, you will likely find einsum to be helpful.
        ...
        """
        # *** ENTER YOUR CODE BELOW *** #
        first_term = torch.einsum('byi,ij,bxj->byx', arc_dep, self.arc_W, arc_head)
        bias_term = torch.einsum('bxi,i->bx', arc_head, self.arc_B)

        arc_tensor = first_term + bias_term.unsqueeze(1)
        return arc_tensor
    ...
    def create_label_layers(self) -> None:
        ...
        * Initialize self.label_B to zeros.
        ...
        self.label_h_mlp = nn.Sequential(
            nn.Linear(self.pt_width, self.cfg.n_labels),
            nn.ReLU(),
            nn.Dropout(self.cfg.dropout),
        ...
        self.label_W = nn.Parameter(torch.empty(self.cfg.n_labels, self.cfg.n_labels, self._n_deprels))
        nn.init.uniform_(self.label_W, -sqrt(3.0 / self.cfg.n_labels), sqrt(3.0 / self.cfg.n_labels))
        
        self.label_h_W = nn.Parameter(torch.empty(self.cfg.n_labels, self._n_deprels))
        nn.init.uniform_(self.label_h_W, -sqrt(3.0 / self.cfg.n_labels), sqrt(3.0 / self.cfg.n_labels))

        self.label_d_W = nn.Parameter(torch.empty(self.cfg.n_labels, self._n_deprels))
        nn.init.uniform_(self.label_d_W, -sqrt(3.0 / self.cfg.n_labels), sqrt(3.0 / self.cfg.n_labels))

        self.label_B = nn.Parameter(torch.zeros(self._n_deprels))
    ...
    def score_labels(self, label_head: Tensor, label_dep: Tensor) -> Tensor:
        ...
        """
        # *** ENTER YOUR CODE BELOW *** #
        first_term = torch.einsum('byi,ijk,bxj->byxk', label_dep, self.label_W, label_head)
        second_term = torch.einsum('bxi,ij->bxj', label_head, self.label_h_W).unsqueeze(1)
        third_term = torch.einsum('byi,ij->byj', label_dep, self.label_d_W).unsqueeze(2)
    
        label_scores = first_term + second_term + third_term + self.label_B.unsqueeze(0).unsqueeze(1).unsqueeze(2)
        return label_scores
    ...
    def mask_possible(self, shape: T.Sequence[int]):
        ...
        # Constraint 2: The root (vertex 0) cannot be a dependent
        mask[:, 0, :, :] = False

        # Constraint 3: Only the root vertex (0) can have relationship label 0 (root dependency relation)
        for j in range(1, x):
            mask[:, :, j, 0] = False
